...
describe('Transform a React Admin request to an Hydra request', () => {
  ...
  const mockApiDocumentationParser = jest.fn(() => Promise.resolve({
    api: new _apiDocParser.Api('entrypoint', {
      resources: [new _apiDocParser.Resource('resource', '/resources', {
        fields: [new _apiDocParser.Field('bar')]
      })]
    })
  }));
  const dataProvider = (0, _dataProvider.default)('entrypoint', mockFetchHydra, mockApiDocumentationParser);
  test('React Admin get list with filter parameters and custom search params', async () => {
    ...
    expect(searchParams[6]).toEqual(['nested_array.nested[0]', '/nested_iri/1']);
    expect(searchParams[7]).toEqual(['nested_array.nested[1]', '/nested_iri/2']);
    expect(searchParams[8]).toEqual(['exists[foo]', 'true']);
    expect(searchParams[9]).toEqual(['nested_date.date[before]', '2000']);
    expect(searchParams[10]).toEqual(['nested_range.range[between]', '12.99..15.99']);
  });
  ...
  test('React Admin create upload file', async () => {
    ...
    expect(Array.from(options.body.entries())).toEqual([['image', file], ['bar', 'baz'], ['array', '["foo","dummy"]'], ['object', '{"foo":"dummy"}'], ['date', '2020-07-06T12:00:00.000Z']]);
  });
  test('React Admin create with file field', async () => {
    ...
    expect(options).toHaveProperty('body');
    expect(options.body).toBeInstanceOf(FormData);
    expect(Array.from(options.body.entries())).toEqual([['bar', 'baz'], ['foo', 'foo']]);
  });
  ...
  test('React Admin update with file field', async () => {
    ...
    expect(options).toHaveProperty('body');
    expect(options.body).toBeInstanceOf(FormData);
    expect(Array.from(options.body.entries())).toEqual([['foo', 'foo'], ['bar', 'baz']]);
  });
});